...
import { CodeAction, CodeActionKind, Diagnostic, TextDocumentEdit, TextEdit, WorkspaceEdit } from "vscode-languageserver";
...
export function FuzzyMatch(builder: CodeActionBuilder, diag: Diagnostic) : Promise<void> {
    ...
    return Database.ForEach((item)=>{
        ...
        items.sort((x, y,)=>x.distance - y.distance).forEach(m =>{
            if (m.id === "") return;
                ...
                edit:{
                    documentChanges:[
                        TextDocumentEdit.create(
                            { uri:builder.doc.uri, version:builder.doc.version },
